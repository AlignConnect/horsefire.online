...
const Form = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            formdata.append("post", "Horsefire.online/hm")
            formdata.append("ip", check ? check : "")
            formdata.append("checkout_toogle", checkoutDetail.checkoutChecked)
            formdata.append("blog_url", process.env.NEXT_APP_URL_HM ?? "horsefire.online/hm")
    ...
    return (
        <div id="form" className={poppin.className}>
            ...
            <div className="md:w-[70%] overflow-hidden mx-auto bg-[#f3f6fb] border border-black rounded-lg my-5" id='formcallhide'>
                <div>

                    <img src="/main/hm_images/dis.jpg" alt="asdas" loading="lazy" className="mb-5" />

                    <div className="text-center">
                        <h1 className={`${noto.className} text-3xl font-semibold inline-flex text-[#04356e]`}>

                            हॉर्सफायर  खरीदने के  लिए  अभी फॉर्म भरे
                    ...
                    <form className="space-y-3 px-4">
                        {CustomerDataValue?.map((data, key) => {
                            return <div className={`${noto.className} flex flex-col my-6`} key={key}>
                                ...
                                <div className="relative">
                                    <input {...data.inputValue} className={` py-3 border border-black w-full rounded-sm focus:outline-none  ${data.icon ? 'pl-[3.5rem]' : 'pl-[0.7rem]'}`} onChange={handleChange} value={CustomerData[data.inputValue.name]} />
                        ...
                        <div className="flex justify-center items-center">
                            <button onClick={handleFormSubmit} type="button" className={`${poppin.className} bg-[#04356f] my-4 text-white text-xl font-semibold p-2 md:w-72 w-full rounded-md hover:bg-yellow-400/80 transition-colors`}>
                                SUBMIT
                            </button>
                        </div>
                    </form>
